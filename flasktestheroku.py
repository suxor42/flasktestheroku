...
def timesincelastrequest():
    value = "Seconds since last shared tracking call: %s" % (datetime.datetime.now() - datetime.datetime.strptime(json.loads(redis.get('lastrequest'))['time'], timeformat)).total_seconds()
    return value
